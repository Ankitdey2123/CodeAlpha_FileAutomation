...
def automation_file():
    global sourceText,f1,source_location
    f1 = Frame(root, width=350, height=300, background="lavender")
    f1.grid(row=5, column=0, columnspan=4)

    source_location = StringVar()

    link_Label = Label(f1, text="Select The File To automate ", font="bold", bg='lavender')
    link_Label.grid(row=0, column=0, pady=5, padx=5)

    sourceText = Entry(f1, width=50, textvariable=source_location, font="12")
    sourceText.grid(row=0, column=1, pady=5, padx=5)
    source_browseButton = Button(f1, text="copy link from here",bg='cyan2', command=source_browse, width=15, font="bold")
    source_browseButton.grid(row=0, column=2, pady=5, padx=5)

    submit_Button = Button(f1, text="submit", bg='dark green',fg='white',command=submit, width=15, font=('bold',12))
...
lbl_heading = Label(root, text="File Automation App",font=("Berlin Sans FB Demi",21,"bold"),fg="deep pink",bg='#E7FFFF')
...
view_btn = Button(root, text="file automation", command=automation_file, width=15,font=('bold',14))
